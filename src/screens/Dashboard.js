...
import { Alloverstudent, useAllOverStudent } from "../hooks/usePost";
...
export default function Dashboard() {
  ...
  const Toaster = () => {
    toast.success("New Staff Register successfully");
  };
  const errtoast = () => {
          ...
          <div className="print-btn flex items-center space-x-3">
            <ReactToPrint
              trigger={() => (
                <button
                  id="print"
                  className="text-3xl bg-class2-50 rounded-md text-white p-1"
                >
                  <MdLocalPrintshop />
                </button>
              )}

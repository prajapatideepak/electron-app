...
const Reciept = () => {
  ...
  let isSalaried = location.state?.isSalaried
    ? location.state?.isSalaried
    : false;
  ...
  useEffect(() => {
    async function getReceiptDetails() {
      if (isStaff) {
        ...
        try {
          let receipt_details = await searchReceipt(
            location.state.fees_receipt_id
          );
          receipt_details = receipt_details.data.student_receipts[0];
          setReceiptDetails(() => {
            let date = new Date(
              receipt_details?.academics[0].fees[0].fees_receipt[0].date
            ).toLocaleString();
            date = date.split(",")[0];

            let amountInWords = inWords(
              receipt_details?.academics[0].fees[0].fees_receipt[0]
            ...
            return {
              ...
              is_by_cash:
                receipt_details?.academics[0].fees[0].fees_receipt[0]
                  .transaction[0].is_by_cash,
              is_by_upi:
                receipt_details?.academics[0].fees[0].fees_receipt[0]
                  .transaction[0].is_by_upi,
              is_by_cheque:
                receipt_details?.academics[0].fees[0].fees_receipt[0]
                  .transaction[0].is_by_cheque,
              upi_no:
                receipt_details?.academics[0].fees[0].fees_receipt[0]
                  .transaction[0].upi_no,
              cheque_no:
                receipt_details?.academics[0].fees[0].fees_receipt[0]
                  .transaction[0].cheque_no,
              amount:
                receipt_details?.academics[0].fees[0].fees_receipt[0]
                  .transaction[0].amount,
              discount:
                receipt_details?.academics[0].fees[0].fees_receipt[0].discount,
              admin:
                receipt_details?.academics[0].fees[0].fees_receipt[0].admin[0]
                  .username,
            };
          });
          setLoading(false);
    ...
    <section className="relative">
      ...
      <div className={`bg-slate-100 ${model && "opacity-20"}`}>
        <div className="flex justify-between pt-8 pb-4 px-6">
          ...
          {location?.state?.prevPath != "/receipt/update/student" &&
          location?.state?.prevPath != "/receipt/FeesDetail" ? (
            <div
